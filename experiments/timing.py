...
if (len(sys.argv) < 2) or (sys.argv[1].lower() == 'noplot'):
    ...
    with open('timing.csv', 'w') as outFile:
        outFile.write('Length,gaussian_cov (Python),parzen (Python),gaussian_cov (libmaxdiv),parzen (libmaxdiv)\n')
...
if (len(sys.argv) < 2) or (sys.argv[1].lower() != 'noplot'):
    ...
    plt.plot(N, times[:, 0] * 1000, 'b--', label = 'gaussian_cov (Python)')
    plt.plot(N, times[:, 1] * 1000, 'r--', label = 'parzen (Python)')
    plt.plot(N, times[:, 2] * 1000, 'b-',  label = 'gaussian_cov (libmaxdiv)')
    plt.plot(N, times[:, 3] * 1000, 'r-',  label = 'parzen (libmaxdiv)')
